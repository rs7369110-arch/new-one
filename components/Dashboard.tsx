...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  return (
    <div className="space-y-3 md:space-y-12 pb-10 overflow-hidden">
      ...
      {/* Ultra Compact Header with Online Status */}
      <header className="relative flex flex-col md:flex-row md:items-center justify-between gap-3 p-3 md:p-12 glass-card rounded-[1rem] md:rounded-[4rem] animate-stagger-1">
         ...
         <div className="z-10 flex justify-between items-center w-full md:w-auto">
            <div>
               ...
               <div className="flex items-center gap-2 mt-1">
                  ...
                  <p className={`${isDarkMode ? 'text-slate-500' : 'text-slate-400'} font-black text-[6px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.6em]`}>
                     {isOnline ? 'Online' : 'Offline'} | {user.name}
                  </p>
               </div>
            </div>
            {/* Mobile Calendar Link/Button */}
            <div className="md:hidden flex items-center gap-2 bg-indigo-500/10 px-2 py-1 rounded-lg border border-indigo-500/20">
               <i className="fa-solid fa-calendar-day text-[10px] text-indigo-400"></i>
         ...
         <div className="z-10 hidden md:flex items-center justify-between md:justify-end gap-2 md:gap-4">
            <div className={`flex-1 md:flex-none px-4 md:px-10 py-2 md:py-5 border rounded-xl md:rounded-[2.5rem] backdrop-blur-3xl transition-all ${
               ...
               <p className="text-[7px] md:text-[10px] font-black text-indigo-400 uppercase tracking-widest text-center">Today</p>
               <p className={`text-xs md:text-lg font-black text-center ${isDarkMode ? 'text-white' : 'text-slate-700'}`}>{new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</p>
            </div>
            <div className={`w-10 h-10 md:w-20 md:h-20 rounded-xl md:rounded-[2.5rem] flex items-center justify-center text-base md:text-3xl shadow-xl text-white floating-icon ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-10">
        ...
        <div className="glass-card p-3 md:p-12 rounded-[1rem] md:rounded-[4.5rem] animate-stagger-3">
          ...
          <div className="space-y-2 md:space-y-6">
            ...
            )) : <div className="py-4 text-center text-slate-600 font-black uppercase tracking-widest opacity-30 text-[7px]">Clean board.</div>}
          </div>
        </div>
      </div>
    </div>
