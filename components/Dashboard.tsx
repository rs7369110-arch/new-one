...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const handleDeleteNotice = (id: string, title: string) => {
    ...
    const confirmed = window.confirm(`DELETE: Are you sure you want to remove "${title}"?`);
    if (confirmed) {
  ...
  return (
    <div className="space-y-4 md:space-y-6 pb-6 overflow-hidden animate-fade-in">
      ...
      <div className={`p-4 md:p-8 rounded-[2rem] md:rounded-[3rem] border-2 flex flex-col md:flex-row items-center text-center md:text-left justify-between relative overflow-hidden transition-all bento-card-3d ${
         ...
         {isAdmin && (
           <div className="grid grid-cols-2 md:flex md:flex-wrap gap-2 md:gap-4 mt-4 md:mt-0 w-full md:w-auto">
             ...
             <button onClick={() => { setBrandForm(safeBranding); setIsBrandingOpen(true); }} className="btn-3d-indigo col-span-2 md:col-span-1 px-3 md:px-8 py-3 md:py-4 bg-indigo-600 text-white rounded-xl md:rounded-2xl text-[8px] md:text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2">
      ...
      {/* BRANDING CONFIG MODAL (Unchanged Logic, added responsive padding) */}
      {isBrandingOpen && (
           ...
           <div className={`w-full max-w-xl rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 relative z-10 shadow-2xl animate-scale-in border-t-[8px] md:border-t-[12px] border-indigo-600 overflow-y-auto max-h-[90vh] ${isDarkMode ? 'bg-[#0f172a]' : 'bg-white'}`}>
              ...
              <form onSubmit={handleSaveBranding} className="space-y-4 md:space-y-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div className="space-y-1">
                       <label className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Phone</label>
                       <input className={`w-full px-4 py-3 md:px-6 md:py-4 rounded-xl md:rounded-2xl border-none outline-none font-bold text-sm md:text-base ${isDarkMode ? 'bg-black text-white' : 'bg-slate-50 text-indigo-900'}`} value={brandForm.phone} onChange={e => setBrandForm({...brandForm, phone: e.target.value})} />
                    </div>
                    <div className="space-y-1">
                       <label className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Theme</label>
                       <div className="flex gap-2">
                          <input type="color" className="w-10 h-10 md:w-12 md:h-14 rounded-lg md:rounded-xl border-none p-1 cursor-pointer bg-transparent" value={brandForm.themeColor} onChange={e => setBrandForm({...brandForm, themeColor: e.target.value})} />
                          <input className={`flex-1 px-4 py-3 md:py-4 rounded-xl md:rounded-2xl border-none outline-none font-black text-xs md:text-sm uppercase ${isDarkMode ? 'bg-black text-white' : 'bg-slate-50 text-indigo-900'}`} value={brandForm.themeColor} onChange={e => setBrandForm({...brandForm, themeColor: e.target.value})} />
                       </div>
                    </div>
                    <div className="md:col-span-2 space-y-1">
                       <label className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Address</label>
                       <textarea rows={2} className={`w-full px-4 py-3 md:px-6 md:py-4 rounded-xl md:rounded-2xl border-none outline-none font-bold text-xs md:text-sm ${isDarkMode ? 'bg-black text-white' : 'bg-slate-50 text-indigo-900'}`} value={brandForm.address} onChange={e => setBrandForm({...brandForm, address: e.target.value})} />
                    </div>
                 </div>
