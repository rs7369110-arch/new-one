...
const Sidebar: React.FC<SidebarProps> = ({ role, activeTab, setActiveTab, onLogout, userName, isDarkMode, toggleTheme, unreadCounts, currentLang, toggleLanguage, isOpen, onClose }) => {
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      ...
      alert("App installation ready! Check your browser address bar icon.");
    }
  };
  ...
  const handleBackup = () => {
    ...
    link.href = url;
    link.download = `digital_edu_setup_${new Date().toISOString().split('T')[0]}.setup`;
    link.click();
    ...
    input.onchange = (e: any) => {
      ...
      reader.onload = (event: any) => {
        try {
          ...
          if (window.confirm("RESTORE DATA: This will overwrite current academy records. Proceed?")) {
    ...
    <>
      ...
      <div className={`fixed md:relative inset-y-0 left-0 w-80 flex flex-col transition-all duration-500 ease-in-out z-[3000] shrink-0 border-r ${
        ...
        <div className="flex-1 py-2 px-6 space-y-2 relative z-10 overflow-y-auto custom-scrollbar">
          ...
          <div className="pt-6 mt-6 border-t border-indigo-500/10 space-y-2">
             <p className="px-6 text-[8px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4">System Utilities</p>
             
             <button 
                onClick={handleInstallClick}
                className={`w-full flex items-center gap-5 px-6 py-4 rounded-[1.8rem] transition-all hover:bg-emerald-500/10 text-emerald-500`}
             >
                <i className="fa-solid fa-mobile-screen-button w-8 text-center text-lg"></i>
                <span className="font-black tracking-widest text-[11px] uppercase">Install App</span>
             </button>
             ...
             {role === UserRole.ADMIN && (
               <div className="grid grid-cols-2 gap-2 px-2">
                  <button 
                    ...
                    className="flex flex-col items-center gap-2 py-4 rounded-3xl bg-indigo-500/5 text-indigo-400 border border-indigo-500/10 hover:bg-indigo-500 hover:text-white transition-all"
                  >
                  ...
                  <button 
                    ...
                    className="flex flex-col items-center gap-2 py-4 rounded-3xl bg-amber-500/5 text-amber-500 border border-amber-500/10 hover:bg-amber-500 hover:text-white transition-all"
                  >
                     <i className="fa-solid fa-file-import text-lg"></i>
                     <span className="text-[7px] font-black uppercase tracking-widest">Restore</span>
                  </button>
               </div>
             )}
          </div>
